...
class MasterHandler(BaseRequestHandler):
    def heartbeat(self, heartbeat):
        data = json.dumps(json.dumps({"version": heartbeat["version"], "timestamp": heartbeat["timestamp"]}))
        self.server.zk.set('/{}/agents/{}'.format(self.server.root, heartbeat['id']), data.encode())
